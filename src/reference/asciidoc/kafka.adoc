...
Starting with version 2.6.8, it has additional methods which are called after the listener exits (normally, or by throwing an exception).
Also, starting with version 2.6.8, there is now a `BatchInterceptor`, providing similar functionality for <<batch-listeners>>.
